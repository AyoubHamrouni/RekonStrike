...
class TargetRepository:
    ...
    async def add_subdomains(self, target_id: int, subdomains: list[str], source: str = "passive"):
        if not subdomains:
            return
        ...
        new_subs = [{"target_id": target_id, "subdomain": s, "source": source} 
                    for s in subdomains if s not in existing]
        
        if new_subs:
            await self.session.execute(insert(Subdomain).values(new_subs))

    async def get_subdomains(self, target_id: int, resolved: Optional[bool] = None, 
                              page: int = 0, size: int = 100) -> tuple[Sequence[Subdomain], int]:
