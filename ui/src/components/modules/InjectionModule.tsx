...
import { ChevronDown, ChevronRight, AlertTriangle, BookOpen, Beaker, Sparkles, Loader2, X } from "lucide-react";
...
function VerifyForm({ host, onSave, onBack }: VerifyFormProps) {
  const [form, setForm] = useState<FindingForm>({ url: "", request: "", response: "", steps: "", impact: "", severity: "critical", title: "" });
...
export default function InjectionModule({ host, targetId }: InjectionModuleProps) {
      ...
      {tab === "discover" && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className={`px-3 py-2 rounded-lg border text-xs text-center ${discoverResults?.withParams > 0 ? "bg-green-subtle border-green/30 text-green" : "bg-surface-2 border-white/5 text-text-dim"}`}>
              <div className="font-medium">{discoverResults?.withParams || 0}</div>
              <div className="text-[10px] mt-0.5">URLs with params</div>
            </div>
            <div className={`px-3 py-2 rounded-lg border text-xs text-center ${discoverResults?.formEndpoints > 0 ? "bg-green-subtle border-green/30 text-green" : "bg-surface-2 border-white/5 text-text-dim"}`}>
              <div className="font-medium">{discoverResults?.formEndpoints || 0}</div>
              <div className="text-[10px] mt-0.5">Form endpoints</div>
            </div>
            <div className="px-3 py-2 rounded-lg border text-xs text-center bg-surface-2 border-white/5 text-text-dim">
