...
import { Card, Badge, IconButton } from "@/components/ui/Shared";
...
export default function Dashboard() {
  const { state, dispatchAgent, reset } = useDashboardAgent();
      ...
      <div className="grid grid-cols-12 gap-6">
        <Card
          ...
          action={
            <div className="flex gap-2 items-center">
              <IconButton icon={<RotateCcw size={14} />} onClick={handleReset} />
              <button
                onClick={handleDispatch}
                disabled={state.status === "running" || state.status === "starting" || state.status === "creating_target"}
                className={`flex items-center gap-2 px-4 py-1 rounded-lg text-[10px] font-black uppercase tracking-tighter transition-all cursor-pointer ${
        ...
        >
            ...
            {showTargetDropdown && existingTargets.length > 0 && (
              <div className="absolute z-20 top-full mt-1 left-0 right-0 bg-slate-900 border border-white/10 rounded-lg shadow-xl max-h-40 overflow-y-auto">
                {existingTargets.map((t) => (
                  <button
                    key={t.id}
                    className="w-full text-left px-4 py-2 text-xs text-slate-400 hover:bg-white/5 hover:text-slate-200 font-mono transition-colors cursor-pointer"
                    onMouseDown={() => {
          ...
          {state.targetDomain && (
            <div className="flex items-center gap-2 mb-3 px-1">
              ...
              {state.status === "completed" && <Badge variant="default">Complete</Badge>}
              {state.status === "error" && <Badge variant="danger">Error</Badge>}
            </div>
          )}
                ...
                <div className="flex flex-col items-center flex-1">
                  ...
                  <span
                    className={`text-[9px] font-black uppercase tracking-widest ${
                      state.currentPhase === phase.key ? "text-purple-400" : "text-slate-600"
                    }`}
                  >
                    {phase.label}
                  </span>
                ...
                {i < PHASES.length - 1 && (
                  <div
                    className={`h-[1px] w-full max-w-[30px] mt-6 transition-colors duration-500 ${
                      state.phasesCompleted.includes(phase.key) ? "bg-purple-600/50" : "bg-slate-800"
                    }`}
                  />
                )}
              </React.Fragment>
        ...
        <div className="col-span-12 lg:col-span-4 grid grid-cols-2 gap-4">
          {[
            ...
            <div
              key={stat.label}
              className={`bg-slate-900/50 border border-white/5 rounded-xl p-5 flex flex-col justify-between hover:bg-slate-900 transition-colors ${(stat as { border?: string }).border || ""}`}
            >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            {host.title && (
                              <span className="text-[10px] text-slate-600 font-medium">{host.title}</span>
                            )}
                          </div>
                        </div>
                      </td>
            ...
            <div className="flex flex-col items-center justify-center py-12 text-slate-700 select-none">
              {state.status === "running" ? (
                <>
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Scanning in progress...</span>
                </>
              ) : (
                <>
      ...
      <div className="grid grid-cols-12 gap-6">
        <Card className="col-span-12 lg:col-span-6" title="Infrastructure Proximity Mapping">
          <div className="h-48 flex items-center justify-center bg-slate-950/40 rounded-xl relative overflow-hidden">
            <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle,rgba(124,58,237,1)_1px,transparent_1px)] bg-[size:20px_20px]" />
